...
from .models import Company, Plan, Subscription, CompanyTeamMember, User, Client
...
class LoginSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not user.is_active:
            raise serializers.ValidationError(
                "This user has been deactivated or has not been activated."
            )
...
class CompanyTeamMemberSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('company', 'email', 'first_name', 'last_name', 'plan')
    ...
    def to_representation(self, obj):
        ...
        return super(CompanyTeamMemberSerializer, self).to_representation(obj)


class AccountSerializer(serializers.Serializer):
    """Serializer that has two nested Serializers: company and companymanager"""
    ...
    def create(self, validated_data):
        ...
        user_data = validated_data['manager']
        ...
        subscription = {
            ...
            timedelta(
                days=free_trial_plan.duration_days)}
        company_plan = Subscription.objects.get_or_create(**subscription)

        # create more users here if required

        # user for the client to log into his business' dashboard

        company_manager = User.objects.create_superuser(
